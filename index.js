...
function deepEqual(a, b) {
 ...
 function getData(metadata,finalData,finalQuery,data, cb) {
	 ...
	 mysqlcon.exec(query.toString(),function (err, result) {
		 ...
		 getRecursionInput(metadata,data, result, function (err, pushThese) {
			 ///console.log(table._name, result);
 ...
 function crawl(schema,constraints,callback){
     /**
...
function main(fkpk,schema,options,cb){
    var opts={
	dbconfig:options.dbconfig	
    };
    async.waterfall([
	async.constant(fkpk,opts),
	function(fkpk,opts,callback){
		...
		var newErr,schema;
			...
			return callback(newErr,fkpk,schema);
		    }		
	    });	    
	},
	function(fkpk,schema,callback){
	    crawl(schema,fkpk,function (err,finalData,finalQuery){
		async.parallel({
		    query: function(callback){writeToFile(finalQuery,'query.sql',callback);}
		},function(err,result){
...
(function () {
	if (require.main == module) {
	    ...
	    var schema = require('./test/sakila/sakila');
	    main(fkpk,schema,{},function (err,result){
